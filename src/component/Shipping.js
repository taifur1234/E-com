    ...
    const cartItems = storedCart
      .map((cartItem) => {
        const product = itemData.find(
          (p) => p.productName === cartItem.productName
        );
        if (!product) return null;
        const oldPriceNum = Number(product.oldPrice.replace(/[^\d.-]/g, ""));
        const discountPriceNum = Number(
          product.discountPrice.replace(/[^\d.-]/g, "")
        );
        const discount = oldPriceNum - discountPriceNum;
        return {
          id: product.id,
          title: product.productName,
          subtitle: product.specifications?.Category || "—",
          ...
          {items.length === 0 ? (
            ...
            items.map((item) => (
              <div className="cart-item-div" key={item.id}>
                ...
                <div className="cart-item-div-info">
          ...
          <div className="popup-box" onClick={(e) => e.stopPropagation()} style={{ width: 480 }}>
            ...
            <div style={{ marginTop: 14, display: "flex", gap: 10, justifyContent: "flex-end" }}>
              ...
              <button
                ...
                disabled={processing}
              >
                {processing ? "Processing..." : `Pay ₹${totalPrice}.00`}
              </button>
            </div>
